  ...
  document.addEventListener("DOMContentLoaded", function () {
    fetch(API_URL)
      ...
      .then(products => {
        ...
        // Carousel: pick up to 8 randomized items, keep arrows intact (only moves .product-card children)
        ...
        if (grid) {
          // remove current direct .product-card children (keeps arrow elements outside #product_grid)
          Array.from(grid.querySelectorAll(":scope > .product-card")).forEach(n => n.remove());
          renderInto("#product_grid", carouselItems, true);
        }

        // Product display: render larger randomized block into #product_display
        const displayItems = shuffleArray(products).slice(0, Math.min(12, products.length)).map(createCardHTML);
        renderInto("#product_display", displayItems);
      })
      .catch(err => {
        console.error("Failed to load products:", err);
      });

    // Delegate add-to-basket clicks to call existing addToCart()
    document.body.addEventListener("click", function (e) {
      ...
      if (typeof addToCart === "function") {
        ...
        // fallback alert if addToCart missing
